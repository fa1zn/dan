...
const TIER_PILL: Record<string, { label: string; cls: string }> = {
  platinum: { label: "✓ Mfr-verified", cls: "bg-violet-100 text-violet-700 dark:bg-violet-950 dark:text-violet-300" },
  gold: { label: "✓ Verified", cls: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" },
  silver: { label: "Unverified", cls: "bg-muted text-muted-foreground" },
  ...
  return (
    <div className="group relative flex flex-col gap-2 rounded-xl border bg-card p-4 transition-colors hover:border-foreground/30">
      <div className="flex items-start justify-between gap-2">
        ...
        {d.oem ? <Badge variant="muted">{d.oem}</Badge> : null}
      </div>
