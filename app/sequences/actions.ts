...
function filterFrom(data: { oem?: string; state?: string; city?: string }): SegmentFilter {
  return {
...
export async function previewSegmentAction(data: { oem?: string; state?: string; city?: string }): Promise<number> {
...
export async function launchSegmentAction(formData: FormData) {
  ...
  if (!f.oem && !f.state && !f.city) return; // refuse to enroll "everything"
