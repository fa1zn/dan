...
function draftOpener(kind: string | undefined, oem: string | null | undefined): string {
  ...
  switch (kind) {
    ...
    case "volume":
      return `With the volume you do, I'd bet a real chunk of calls hit voicemail after close, and that's exactly the lead leak we plug for ${b}dealers.`;
    case "quality":
      return `Saw a few reviews mention slow callbacks, so we make sure no lead sits waiting at ${b}stores. Could I grab ten minutes?`;
    case "greenfield":
    default:
...
export function computePamFit(input: PamFitInput): PamFit {
  ...
  // Size / value
  if (input.tier === "A") add("Tier A, group or multi-store", 10);
