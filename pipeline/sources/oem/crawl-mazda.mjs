/*
 * MAZDA OEM SPINE CRAWLER (CA/TX/FL MVP) — Mazda's locator API is clean & public:
 * mazda.com/services/dealers/distances/by/zipcode. Returns dealer.id (code), name,
...
async function main() {
  ...
  console.log(`\n▶ MAZDA OEM CRAWL (CA/TX/FL) — ${ZIPS.length} zip anchors`);
  ...
  await page.goto("https://www.mazdausa.com/", { waitUntil: "domcontentloaded", timeout: 60000 });
  await sleep(2500);
  ...
  for (let i = 0; i < ZIPS.length; i++) {
    try {
      const dealers = await page.evaluate(async (zip) => {
        const r = await fetch(`https://www.mazdausa.com/handlers/dealer.ajax?zip=${zip}&maxResults=60&radius=120`, { headers: { Accept: "application/json" } });
        if (!r.ok) return { _status: r.status };
        return (await r.json()).body?.results || [];
      }, ZIPS[i]);
    ...
    if ((i + 1) % 20 === 0 || i === ZIPS.length - 1) console.log(`  [${i + 1}/${ZIPS.length}] unique Mazda dealers: ${byCode.size}`);
    await sleep(700);
  }
  await browser.close();

  const existing = db.prepare("SELECT id, name, city, phone, latitude, longitude FROM dealerships WHERE oem='Mazda'").all();
  ...
  const upd = db.prepare(`UPDATE dealerships SET dealer_code=@code, brand_confirmed=1, trust_tier='platinum', phone=COALESCE(phone,@phone),
      ...
      source=CASE WHEN source LIKE '%oem:mazda%' THEN source ELSE source||'+oem:mazda' END, updated_at=CURRENT_TIMESTAMP WHERE id=@id`);
  const ins = db.prepare(`INSERT INTO dealerships (name,oem,address_street,city,state_province,postal_code,country,territory,latitude,longitude,phone,website,source,brand_confirmed,trust_tier,dealer_code,dedup_key,created_at,updated_at)
      VALUES (@name,'Mazda',@street,@city,@state,@zip,'US',@territory,@lat,@lng,@phone,@web,'oem:mazda',1,'platinum',@code,@dedup,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`);

  let confirmed = 0, inserted = 0;
  for (const d of byCode.values()) {
    if (!ALLOW.has(d.state)) continue;
    const lat = d.lat != null ? +d.lat : null, lng = d.long != null ? +d.long : null;
    const phone = fmtPhone(d.dayPhone || d.prefPhone);
    const m = (lat != null && near(lat, lng)) || (phone && byPhone.get(digits(phone)));
    if (m) { upd.run({ id: m.id, code: d.id, phone, street: d.address1 || null, zip: d.zip || null, web: d.webUrl || null, lat, lng }); confirmed++; }
    else { try { ins.run({ name: d.name, street: d.address1 || null, city: d.city || null, state: d.state, zip: d.zip || null, territory: REGION[d.state] || "US-Other", lat, lng, phone, web: d.webUrl || null, code: d.id, dedup: `mazda|${d.id}` }); inserted++; } catch {} }
  }
  console.log(`\n  ✓ Mazda: ${byCode.size} pulled → ${confirmed} upgraded, ${inserted} net-new — all Platinum (dealer codes)`);
  console.log(`  Mazda CA/TX/FL manufacturer-confirmed: ${db.prepare("SELECT COUNT(*) n FROM dealerships WHERE oem='Mazda' AND state_province IN ('CA','TX','FL') AND brand_confirmed=1").get().n}`);
}
