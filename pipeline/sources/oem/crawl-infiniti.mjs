/*
 * INFINITI OEM CRAWL (CA/TX/FL) — Infiniti is a Nissan-family brand and its retailer
 * locator (infinitiusa.com/locate-infiniti-retailer.html) uses the SAME AppSync GraphQL API:
 *   POST https://graphql.nissanusa.com/graphql  (x-api-key: da2-3gadjirlkbfdzg7fiosg4tewl4)
 *   getDealersByLatLng with market.brand="infiniti" →
 *     { id (dealer code, e.g. "NNA73101"), name, address{addressLine1,city,stateCode,postalCode},
 *       phoneNumber, websiteURL, geolocation{latitude,longitude} }
 * Lat/lng grid over CA/TX/FL, dedup by id, filter to CA/TX/FL, match existing by geo→phone → Platinum.
 */
...
const STEP = Number(process.env.CRAWL_STEP || 1.4);
...
async function main() {
  ...
  const pts = grid(STEP);
  console.log(`\n▶ INFINITI OEM CRAWL (CA/TX/FL) — ${pts.length} grid points (step ${STEP}°), GraphQL brand=infiniti`);
  ...
  await page.goto("https://www.infinitiusa.com/locate-infiniti-retailer.html", { waitUntil: "domcontentloaded", timeout: 60000 });
  await sleep(4000);
  ...
  for (let i = 0; i < pts.length; i++) {
    ...
    try {
      const dealers = await page.evaluate(async ({ lat, lng, apiKey, query }) => {
        const body = { operationName: "getDealersByLatLng", variables: { market: { lang: "en", region: "us", brand: "infiniti", application: "inventory" }, location: { latitude: lat, longitude: lng }, size: 60, radius: 200, isMarketingDealer: false }, query };
    ...
    if ((i + 1) % 25 === 0 || i === pts.length - 1) console.log(`  [${i + 1}/${pts.length}] unique Infiniti dealers: ${byCode.size}`);
    await sleep(500);
  }
  await browser.close();

  const existing = db.prepare("SELECT id, name, city, phone, latitude, longitude FROM dealerships WHERE oem='Infiniti'").all();
  ...
  const upd = db.prepare(`UPDATE dealerships SET dealer_code=@code, brand_confirmed=1, trust_tier='platinum', phone=COALESCE(phone,@phone),
      ...
      source=CASE WHEN source LIKE '%oem:infiniti%' THEN source ELSE source||'+oem:infiniti' END, updated_at=CURRENT_TIMESTAMP WHERE id=@id`);
  const ins = db.prepare(`INSERT INTO dealerships (name,oem,address_street,city,state_province,postal_code,country,territory,latitude,longitude,phone,website,source,brand_confirmed,trust_tier,dealer_code,dedup_key,created_at,updated_at)
      VALUES (@name,'Infiniti',@street,@city,@state,@zip,'US',@territory,@lat,@lng,@phone,@web,'oem:infiniti',1,'platinum',@code,@dedup,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`);
  ...
  for (const d of byCode.values()) {
    ...
    else { try { ins.run({ name: d.name, street: (a.addressLine1 || "").trim() || null, city: a.city || null, state: st, zip: a.postalCode || null, territory: REGION[st] || "US-Other", lat, lng, phone, web, code: d.id, dedup: `infiniti|${d.id}` }); inserted++; } catch {} }
  }
  console.log(`\n  ✓ Infiniti: ${byCode.size} pulled → ${confirmed} upgraded, ${inserted} net-new — all Platinum (dealer codes)`);
  console.log(`  Infiniti CA/TX/FL manufacturer-confirmed: ${db.prepare("SELECT COUNT(*) n FROM dealerships WHERE oem='Infiniti' AND state_province IN ('CA','TX','FL') AND brand_confirmed=1").get().n}`);
}
